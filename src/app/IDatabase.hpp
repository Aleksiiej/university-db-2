...
class IDatabase {
public:
  virtual void addStudent(const std::string &name, const std::string &surname,
                          const std::string &adress, const int &index,
                          const std::string &PESEL, const Sex &sex,
                          const Position &position) noexcept = 0;
  virtual void addEmployee(const std::string &name, const std::string &surname,
  ...
  virtual std::vector<std::shared_ptr<Person>>
  findBySurname(const std::string &surname) const noexcept = 0;
  virtual std::shared_ptr<Person>
  findByPESEL(const std::string &PESEL) const noexcept = 0;
  ...
  virtual bool modifySalary(const std::string &PESEL,
                            const float &newSalary) noexcept = 0;
  virtual bool validatePESEL(const std::string &PESEL) const noexcept = 0;
  virtual void
  loadFromFile(const std::string &fileName = "../database.txt") noexcept = 0;
  virtual void
  saveToFile(const std::string &fileName = "../database.txt") noexcept = 0;
  virtual void generateData(const int &n) noexcept = 0;
  virtual std::shared_ptr<Person>
  getPtrToRecord(const int &pos) const noexcept = 0;
};
