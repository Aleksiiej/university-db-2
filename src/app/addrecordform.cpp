...
AddRecordForm::AddRecordForm(QWidget *parent)
    : QDialog(parent), ui(std::make_unique<Ui::AddRecordForm>()), PESELValidatorPtr_(std::make_unique<PESELValidator>())
{
    ui->setupUi(this);
...
void AddRecordForm::on_pushButton_addrecord_clicked()
{
    ...
    {
        ptrToMainWindow_->getPtrToDatabase()->addStudent(name.toStdString(), surname.toStdString(),
                                                         address.toStdString(), studentEmployeeValue.toFloat(),
                                                         pesel.toStdString(), sex, position);
    }
    else
    {
        ptrToMainWindow_->getPtrToDatabase()->addEmployee(name.toStdString(), surname.toStdString(),
                                                          address.toStdString(), studentEmployeeValue.toFloat(),
                                                          pesel.toStdString(), sex, position);
    }
