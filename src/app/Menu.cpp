...
void Menu::enterData() const noexcept {
  ...
  std::cout
      << "Enter position: type 0 if student, any other button if employee: ";
  ...
  switch (position) {
  case 0:
    ptrToDb_->addStudent(name, surname, adress, index, PESEL,
                         static_cast<Sex>(sex), Position::Student);
    break;

  case 1:
    ptrToDb_->addEmployee(name, surname, adress, salary, PESEL,
                          static_cast<Sex>(sex), Position::Employee);
    break;
  }
}
...
void Menu::modifySalary() const noexcept {
  ...
  if (value == true) {
    system("clear");
    std::cout << "New salary set at value: " << std::fixed
              << std::setprecision(2) << tempSalary << std::endl;
    std::cout << "Press enter to proceed...";
    std::getchar();
  } else if (value == false) {
    system("clear");
