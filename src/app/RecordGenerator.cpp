...
std::unique_ptr<Person> RecordGenerator::generateRandomRecord() const noexcept
{
    ...
    case 0:
        tempPtr =
            std::make_unique<Student>(Student{tempName, tempSurname, generateRandomAdress(), generateRandomIndex(),
                                              generateRandomPESEL(tempSex), tempSex, Position::Student});
        break;

    case 1:
        tempPtr =
            std::make_unique<Employee>(Employee{tempName, tempSurname, generateRandomAdress(), generateRandomSalary(),
