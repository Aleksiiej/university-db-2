...
bool PESELValidator::validatePESELSize(
    const std::string &PESEL) const noexcept {
  if (PESEL.size() != 11) {
    return false;
  }
  return true;
...
bool PESELValidator::validatePESELDate(
    const std::string &PESEL) const noexcept {
...
bool PESELValidator::validateControlNumber(
    const std::string &PESEL) const noexcept {
