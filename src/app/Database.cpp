...
void
Database::addStudent(const std::string& name,
...
void
Database::addEmployee(const std::string& name,
                      const std::string& surname,
                      const std::string& adress,
                      const float& salary,
                      const std::string& PESEL,
...
void
Database::printByPtr(const std::shared_ptr<Person> ptr) const noexcept
{
  std::cout << "===================================" << std::endl;
...
std::string
Database::returnRecordsAsString()
{
...
std::string
Database::returnRecordsWithGivenSurname(std::string surname)
{
  std::stringstream sstream;
  for (const auto& el : database_) {
...
std::string
Database::returnRecordWithGivenPesel(std::string pesel)
{
  std::stringstream sstream;
  for (const auto& el : database_) {
...
void
Database::showBySurname(const std::string& surname) const noexcept
{
  system("clear");
...
void
Database::showByPESEL(const std::string& PESEL) const noexcept
{
  system("clear");
...
{
  auto tempIter =
    std::find_if(begin(database_), end(database_), [&PESEL](const auto el) {
...
void
Database::sortBySurname() noexcept
{
...
void
Database::sortBySalary() noexcept
{
  std::sort(begin(database_), end(database_), [](auto lhs, auto rhs) {
...
std::string
Database::returnEmployeesSortedBySalary()
{
  ...
  std::sort(begin(ret), end(ret), [](auto lhs, auto rhs) {
    return lhs->getSalary() < rhs->getSalary();
  });
  std::stringstream sstream;
...
bool
Database::checkIfGivenIndexExist(const int& index)
{
  for (const auto& record : database_) {
    if (record->getPosition() == Position::Student and
...
bool
Database::removeByIndex(const int& index) noexcept
{
  if (database_.size() == 0) {
...
bool
Database::modifySalary(const std::string& PESEL,
...
bool
Database::validatePESEL(const std::string& PESEL) const noexcept
{
  ...
  for (const auto& person : data) {
    ...
    if (position == Position::Student) {
      ...
      addStudent(person.at("name").get<std::string>(),
                 ...
                 person.at("address").get<std::string>(),
                 index,
                 person.at("PESEL").get<std::string>(),
                 person.at("sex").get<Sex>(),
                 position);
...
void
Database::saveToJson() const noexcept
{
...
void
Database::toJson(nlohmann::json& j, Person& person) const noexcept
{
  j = nlohmann::json{
...
void
Database::generateData(const int& n) noexcept
{
