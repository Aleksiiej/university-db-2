...
protected:
  void SetUp() {
    database.addStudent("Jan", "Nowak", "Gniezno, Jemiolowa 16", 217025,
                        "93121312345", Sex::male, Position::Student);
    database.addStudent("Aleksandra", "Nowakowska", "Bydgoszcz, Zwyczajna 5",
                        126574, "93040523495", Sex::female, Position::Student);
    database.addStudent("Piotr", "Kowalski", "Warszawa, Przykladowa 20", 223412,
                        "89275430298", Sex::male, Position::Student);
    database.addEmployee("Renata", "Zaradna",
                         "Ustrzyki, Powstancow Malopolskich 133", 4600.50,
                         "73031054872", Sex::female, Position::Employee);
    database.addEmployee("Andrzej", "Niezbedny",
                         "Sopot, Powstancow Slaskich 10", 4340.00,
                         "78032559472", Sex::male, Position::Employee);
  }

  Database database;
  std::shared_ptr<Student> testStudent = std::make_shared<Student>(
      "Aleksandra", "Nowakowska", "Bydgoszcz, Zwyczajna 5", 126574,
      "93040523495", Sex::female, Position::Student);
  std::shared_ptr<Employee> testEmployee = std::make_shared<Employee>(
      "Andrzej", "Niezbedny", "Sopot, Powstancow Slaskich 10", 4340.00,
      "78032559472", Sex::male, Position::Employee);

  bool studentCompare(std::shared_ptr<Person> lhs,
                      std::shared_ptr<Person> rhs) {
    if (lhs->getName() == rhs->getName() &&
        lhs->getSurname() == rhs->getSurname() &&
        lhs->getAddress() == rhs->getAddress() &&
        lhs->getPESEL() == rhs->getPESEL() && lhs->getSex() == rhs->getSex() &&
  ...
  bool employeeCompare(std::shared_ptr<Person> lhs,
                       std::shared_ptr<Person> rhs) {
    if (lhs->getName() == rhs->getName() &&
        lhs->getSurname() == rhs->getSurname() &&
        lhs->getAddress() == rhs->getAddress() &&
